...
class MediaFactory extends Factory
{
    public function definition(): array
    {
        ...
        return [
            ...
            'original_name' => fake()->slug(2).($type === 'image' ? '.jpg' : '.mp4'),
            'type' => $type,
            'disk' => 's3',
            'path' => 'media/'.fake()->uuid().($type === 'image' ? '.jpg' : '.mp4'),
            'thumbnail_path' => null,
            'mime_type' => $type === 'image' ? 'image/jpeg' : 'video/mp4',
            'size' => fake()->numberBetween(100000, 50000000),
            'duration' => $type === 'video' ? fake()->randomFloat(1, 5, 300) : null,
            'width' => $type === 'image' ? fake()->randomElement([1920, 1280, 3840]) : null,
            'height' => $type === 'image' ? fake()->randomElement([1080, 720, 2160]) : null,
            'status' => 'ready',
            'metadata' => null,
        ];
    }
