...
import { Check, Film, Image, Plus, Search } from 'lucide-react';
...
export function MediaPicker({ playlist, availableMedia }: Props) {
    ...
    playlist.items?.forEach((item) => {
        addedCounts.set(item.media.id, (addedCounts.get(item.media.id) ?? 0) + 1);
    });
        ...
        <div className="space-y-3">
            ...
            <div className="max-h-64 space-y-1 overflow-y-auto">
                {filtered.length === 0 ? (
                    ...
                    filtered.map((media) => {
                        const count = addedCounts.get(media.id) ?? 0;
                        ...
                        return (
                            <div
                                ...
                                className={cn(
                                    ...
                                    isAdded ? 'bg-muted/30' : 'hover:bg-muted/50',
                                )}
                            >
                                {/* Thumbnail */}
